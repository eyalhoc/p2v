...
class p2v_connect():
    ...
    def __init__(self, parent, modname, signals, params={}):
        self._parent = parent
    ...
    def _connect_clocks(self, pin, wire, kind):
        self._parent._assert(type(pin) is clock, f"trying to connect clock {wire} to a non clock signal {pin}", fatal=True)
        self._parent._assert(type(wire) is clock, f"trying to connect a non clock signal {wire} to clock {pin}", fatal=True)
    ...
    def _connect(self, pin, wire, kind):
        if type(pin) is clock or type(wire) is clock:
            self._connect_clocks(pin, wire, kind)
        else:
            self._parent._assert(type(pin) is str, f"pin {pin} is of type {misc._type2str(type(pin))} while expecting type str", fatal=True)
            ...
            if kind == "parameter":
                ...
                self._parent._assert(type(wire) is str, f"wire {wire} is of type {misc._type2str(type(wire))} while expecting type str", fatal=True)
        ...
        for name in self._signals:
            ...
            if name not in self._pins:
                ...
                if signal.kind == "input":
                    if ports:
                        if wire in self._parent._signals and self._parent._signals[name].kind == "input":
                            pass
                        else:
                    ...
                    if ports:
                        if wire in self._parent._signals and self._parent._signals[name].kind == "output":
                            pass
                        else:
                    ...
                    if ports:
                        if wire in self._parent._signals and self._parent._signals[name].kind == "inout":
                            pass
                        else:
