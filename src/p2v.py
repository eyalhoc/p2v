...
class p2v():
    ...
    def _get_gen_args(self, top_class, params=None):
        if params is None:
            params = {}
        if hasattr(top_class, "gen") and isinstance(getattr(top_class, "gen"), FunctionType) and (self.tb.seed != 1):
    ...
    def _check_declared(self, wire, allow=False):
        for name in self._get_names(wire):
            if (allow or not self._parse) and name not in self._signals:
                return False
